bayes_mixture_ergm_SR <- function(dat=sim_networks,
                                  sim.k=sim.k,
                                  form=form_sim,
                                  p=p,
                                  K=cluster_num*2,
  ...
  for(i in 1:n){
    ...
    #temp.form <- statnet.common::nonsimp_update.formula(form, temp.dat~.) # LHS of formula is temp.dat
  ...
  #after finish calculating, take the burn in and thinning to get the K and Z with the goal of cluster analysis
  ...
  Khat_table = table(factor(K_nonzero.thin,levels=1:K))
  Khat <- as.numeric(which(Khat_table == max(Khat_table))[1])
  
  samp.end.time <- Sys.time()
  time_used <-samp.end.time-samp.start.time
  
  output1 <- c(K_est_m,Khat,RI,VI)
  output2 <- list(K_est=K_nonzero, z_est=Z, theta_est=theta,w_est=tau,time_used=time_used)
  cat("Yin_no_order","K_est_m",K_est_m,"K_mode",Khat,"RI",RI,"VI",VI, "\n")
...
bayes_mixture_ergm_SR_Kcent <- function(dat=sim_networks,
                                        sim.k=sim.k,
                                  form=form_sim,
                                  p=p,
                                  K=cluster_num*2,
  ...
  output1 <- rbind(c(K_est_m,Khat,RI,VI),c(K_est_m,Khat,RI2,VI2))
  ...
  cat("Yin_kcent","K_est_m",K_est_m,"K_mode",Khat,"RI",RI,"VI",VI, "\n")
  cat("Yin_kcent","K_est_m",K_est_m,"K_mode",Khat,"RI",RI2,"VI",VI2, "\n")
  cat("time_used", time_used,"\n")
  list(output1=output1,output2=output2)

}
bayes_mixture_ergm_SR_empty <- function(dat=sim_networks,
                                        sim.k=sim.k,
                                  form=form_sim,
                                  p=p,
                                  K=cluster_num*2,
    ...
    for(k in nK){
      ...
      if(length(k_id) >= 1){
        ...
        if(beta.theta >= log(runif(1))){
          theta[j,,k] <- theta1
          acc.counts[j,k] <- TRUE
        } else { theta[j,,k] <- theta[j-1,,k] }
      }else{ theta[j,,k]<- rmvnorm( 1, mean= prior.mean, sigma = prior.sigma)}
    }
  ...
  output1 <- c(K_est_m,Khat,RI,VI)
  output2 <- list(K_est=K_nonzero, z_est=Z, theta_est=theta,w_est=tau,time_used=time_used)
  #saveRDS(output,paste("U:/DPMERGM_SN_Simulations/Output/","N",N,"Nnode",Nnode,"_",rp,".rds",sep=''))
  #cat("Yin_empty","K_est_m",K_est_m,"RI",RI,VI, "\n")
  cat("Yin_empty","K_est_m",K_est_m,"K_mode",Khat,"RI",RI,"VI",VI, "\n")
  cat("time_used", time_used,"\n")
  list(output1=output1,output2=output2)
}
